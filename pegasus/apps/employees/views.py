...
from apps.ads_api.models import Profile, Relevance
...
def _add_edit_employee(request, employee: Employee = None):  # type: ignore
    if employee:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            employee.user = request.user  # type: ignore
            employee.save()  # type: ignore
...
@login_required
def profiles_list_htmx(request):
    return render(
        request,
        "pegasus/employees/htmx_profiles_list.html",
        {
            "active_tab": "object_lifecycle",
            "profiles": Profile.objects.filter(managed=True).order_by("nickname"),
        },
    )


def create_profiles_table(request, managed_bool="Yes"):
    managed_bool = request.POST.get("id")
    managed_bool = True if managed_bool == "Yes" else False
    return render(
        request,
        "pegasus/employees/profiles_table.html",
        {
            "profiles": Profile.objects.filter(managed=managed_bool).order_by("nickname"),
        },
    )


def relevant_table(request):
    return render(
        request,
        "pegasus/employees/related_table.html",
        {
            "relevants": Relevance.objects.all(),
        },
    )


def set_relevant(request, rel_id, relevant_bool):
    rel = Relevance.objects.get(pk=rel_id)
    rel.relevant = True if relevant_bool == "True" else False
    rel.save()

    return render(
        request,
        "pegasus/employees/htmx/relevancy_checkbox.html",
        {
            "relevant": rel,
            "checkbox": relevant_bool,
        },
    )


def set_relevant_table(request, table_type: str):
    if table_type == "show_all":
        relevants = Relevance.objects.all()
    else:
        relevants = Relevance.objects.filter(relevant=False, checked=False)

    return render(
        request,
        "pegasus/employees/related_table_loop.html",
        {
            "relevants": relevants,
        },
    )
...
class EmployeeDataAPIView(APIView):
    ...
    def get(self, request):
        ...
        data = user.employees.values("department").annotate(  # type: ignore
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return self.request.user.employees.all()  # type: ignore
